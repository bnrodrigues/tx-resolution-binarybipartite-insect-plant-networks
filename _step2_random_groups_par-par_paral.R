...
# salva as 4 var de interesse
save(list = c("listMtMdOrigSemProblem","arqs","gps","webType"), 
     file = paste0("txtoutput/","_ws4var.RData") )
# limpa as var 
rm(list = ls())

# carrega as 4 var de interesse
load( paste0("txtoutput/","_ws4var.RData") )
...
cl <- makeCluster(P)
# cl eh uma lista, cada elemento Ã© um nÃ³. Ã uma lista
...
geraMtRand <- function(z,temp,arq,gp) {
	...
	#--------------------------------------------------------------------------
    # matriz aleatoria (par a par)
	# colunas
	if(loops1 > 0){
	  for(i in 1:loops1){
		# par aleatorio
		a <- sample(1:length(temp[1,]),2, replace = F) # selecao aleatoria de col
		sumCol <- (apply(temp[,a],1,sum)>0)*1 # soma das colunas e torna binaria
		b <- paste(colnames(temp[,a])[1],colnames(temp[,a])[2]) # compoe a coluna
		temp <- cbind(temp[,-a],sumCol) # sem a colunas e add a nova coluna
		colnames(temp)[length(temp[1,])] <- b
	  }
	}
	# linhas
	if(loops2 > 0){
	  for(i in 1:loops2){
		# par aleatorio
		a <- sample(1:length(temp[,1]),2, replace = F) # qtdd de linhas
		sumCol <- (apply(temp[a,],2,sum)>0)*1 # soma das linhas e torna binaria
		b <- paste(rownames(temp[a,])[1],rownames(temp[a,])[2]) # compoe a linha
		temp <- rbind(temp[-a,],sumCol) # sem a colunas e add a nova coluna
		rownames(temp)[length(temp[,1])] <- b
	  }
	}
    #--------------------------------------------------------------------------
  ...
  for(j in subconj){
    ...
    if(loops1+loops2 > 0){
      ...
      ## z ou qtRandMt ## quantidade de redes aleatorias
        ...
        # paralelizada. Depois sÃ³ concatena os resultados das mt aleatorias
...
numMtRand == contNumRandMt + qtRandMt*contNumRandNA
